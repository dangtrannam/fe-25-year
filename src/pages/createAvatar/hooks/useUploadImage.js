...
export const useCardUploadImage = (canvasRef, userImageRef, userImageSrc, dependencies) => {
    useEffect(() => {
        if (!canvasRef) return;
        ...
        const userImage = async () => {
            ...
            if (userImageSrc) {
                ...
                // Đảm bảo hình ảnh người dùng nằm dưới frame
                const frameImage = canvasRef.getObjects().find(obj => obj.name === 'cardframe');
                if (frameImage) {
                    canvasRef.sendToBack(img);
                    canvasRef.bringToFront(frameImage);
                }
